  ...
  void Update() {
    ...
    if (delay < delayCounter) {
      if (!theEHM.squish && !theEHM.spear) {
        ...
        if (small && !small2) {
          GetComponent < Animator > ().Play("RadishFall");
          small2 = true;
        }
        ...
        if (!small) {
          ...
          if (counter == 0 && transform.position.y - originalPos < 30) {
            theRB.velocity = new Vector2(0, 20);
          } else {
            counter = 1;
          }
          ...
          if (theRB.velocity.y == 0) {
            GetComponent < Animator > ().Play("RadishNormal");
          }
        ...
        if (small2) {
          GetComponent < Animator > ().Play("RadishSquish");
        }
        small = true;
      } else {
        if (!speared) {
